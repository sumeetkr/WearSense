 ...
 * Coped from http://i-liger.com/sites/default/files/ExtAudioRecorder.java
 */
...
public class ExtAudioRecorder
{
    ...
    */
    private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener()
    {
        ...
        {
            int numSamplesRead = audioRecorder.read(buffer, 0, buffer.length); // Fill buffer
            ...
            short[] bufferInShorts;
            try
            {
                if(randomAccessWriter != null) randomAccessWriter.write(buffer); // Write buffer to file
                payloadSize += buffer.length;
                if (bSamples == 16)
                {
                    bufferInShorts = new short[buffer.length / 2];
                    ...
                    { // 16bit sample size
                        ...
                        bufferInShorts[i] = curSample;
                    }
                }
                else
                { // 8bit sample size
                    bufferInShorts = new short[buffer.length];
    ...
    public void dataArrival(AudioReadResult result){
        for(AudioDataArrivedEventListener collector: dataCollectors){
            collector.onNewDataArrived(result);
        }
        ...
        try
        {
            if (state == State.INITIALIZING)
            {
                if (rUncompressed)
                {
                    ...
                    else if((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED))
                    {
                        buffer = new byte[framePeriod*bSamples/8*nChannels];
                        state = State.READY;
                    }
